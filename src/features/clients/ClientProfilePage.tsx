...
export function ClientProfilePage() {
  ...
  const autoGeneratedInterestClientIds = useRef(new Set<string>());
  ...
  useEffect(() => {
    if (
      !clientId ||
      interestStatusLoading ||
      interestStatusError ||
      generateInterestMutation.isPending ||
      (interestStatus?.dueInterestCents ?? 0) <= 0 ||
      autoGeneratedInterestClientIds.current.has(clientId)
    ) {
      return;
    }

    autoGeneratedInterestClientIds.current.add(clientId);
    generateInterestMutation.mutate();
  }, [
    clientId,
    generateInterestMutation,
    interestStatus?.dueInterestCents,
    interestStatusError,
    interestStatusLoading,
  ]);
